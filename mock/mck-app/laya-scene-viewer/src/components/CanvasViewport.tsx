...
export function CanvasViewport({ sceneData, selectedHit, onSelectNode, version }: CanvasViewportProps) {
  ...
  useEffect(() => {
    ...
    const drawNode = (node: SceneNode, px: number, py: number) => {
      ...
      if (node.type === 'Scene') {
        ...
        let ax = 0;
        if (p.align === 'center') { ctx.textAlign = 'center'; ax = w / 2; }
        else if (p.align === 'right') { ctx.textAlign = 'right'; ax = w; }
  ...
  return (
    <div ref={containerRef} style={{ flex: 1, position: 'relative', overflow: 'hidden', backgroundColor: '#1e1e1e' }}>
      <canvas
        ref={canvasRef}
        style={{
